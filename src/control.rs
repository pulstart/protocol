...
impl StreamConfig {
    /// Default Opus frame duration when a peer omits the field.
    const DEFAULT_PACKET_DURATION_MS: u8 = 20;
...
pub struct ClientDisplayInfo {
    ...
    /// The client's display can present HDR (D2). The server AND-gates HDR on
    /// this so a non-HDR client never gets a washed-out BT.2020+PQ stream.
    pub hdr_display: bool,
}

impl ClientDisplayInfo {
    const HDR_DISPLAY_BIT: u8 = 1 << 0;
    ...
    fn serialize(&self) -> [u8; CLIENT_DISPLAY_INFO_PAYLOAD_SIZE] {
        ...
        buf[8] = if self.hdr_display {
            Self::HDR_DISPLAY_BIT
        } else {
            0
        };
        buf
...
pub struct TransportFeedback {
    ...
    /// First/last frame id in the most recent contiguous lost-frame range, used
    /// by the server to pick a targeted recovery tier (A). `(0, 0)` = none.
    pub lost_frame_first: u32,
    pub lost_frame_last: u32,
    /// Smoothed round-trip time in ms (0 = unknown).
    pub rtt_ms: u32,
    /// One-way-delay trend over the window in microseconds: a sustained positive
    /// value means the bottleneck queue is building (pre-loss congestion, B1).
    pub owd_trend_us: i32,
    /// Measured received video throughput in kbps over the window (B1 clamp).
    pub recv_video_kbps: u32,
}
...
impl TransportFeedback {
    fn serialize(&self) -> [u8; TRANSPORT_FEEDBACK_PAYLOAD_SIZE] {
        ...
        buf[24..28].copy_from_slice(&self.lost_frame_first.to_be_bytes());
        buf[28..32].copy_from_slice(&self.lost_frame_last.to_be_bytes());
        buf[32..36].copy_from_slice(&self.rtt_ms.to_be_bytes());
        buf[36..40].copy_from_slice(&self.owd_trend_us.to_be_bytes());
        buf[40..44].copy_from_slice(&self.recv_video_kbps.to_be_bytes());
        buf
    }
...
pub enum ControlMessage {
    ...
    AuthResult(bool),
    /// Bidirectional: sender offers a file transfer.
    FileOffer {
        transfer_id: u32,
        file_size: u64,
        file_name: String,
    ...
    /// Bidirectional: sender signals all chunks sent + integrity hash.
    FileComplete {
        transfer_id: u32,
        total_chunks: u32,
        sha256: [u8; 32],
    ...
    SelectOutput(u32),
    /// Client → server: declared maximum bitrate ceiling in kbps, sent right
...
impl ControlMessage {
    ...
    const TYPE_FILE_OFFER: u8 = 22;
    const TYPE_FILE_ACCEPT: u8 = 23;
    const TYPE_FILE_CHUNK: u8 = 24;
    ...
    pub fn serialize(&self) -> Vec<u8> {
        match self {
            ...
            ControlMessage::FileOffer {
                ...
                let name_len = name_bytes.len().min(MAX_CONTROL_PAYLOAD - 14);
                let payload_len = 4 + 8 + 2 + name_len;
                let mut buf = vec![0u8; CONTROL_HEADER_SIZE + payload_len];
                buf[0] = Self::TYPE_FILE_OFFER;
                buf[1..3].copy_from_slice(&(payload_len as u16).to_be_bytes());
                let p = CONTROL_HEADER_SIZE;
                buf[p..p + 4].copy_from_slice(&transfer_id.to_be_bytes());
                buf[p + 4..p + 12].copy_from_slice(&file_size.to_be_bytes());
                buf[p + 12..p + 14].copy_from_slice(&(name_len as u16).to_be_bytes());
                buf[p + 14..p + 14 + name_len].copy_from_slice(&name_bytes[..name_len]);
                buf
            }
            ControlMessage::FileAccept {
                ...
                let mut buf = vec![0u8; CONTROL_HEADER_SIZE + 5];
                buf[0] = Self::TYPE_FILE_ACCEPT;
                buf[1..3].copy_from_slice(&5u16.to_be_bytes());
                let p = CONTROL_HEADER_SIZE;
                buf[p..p + 4].copy_from_slice(&transfer_id.to_be_bytes());
                buf[p + 4] = if *accepted { 1 } else { 0 };
                buf
            }
            ControlMessage::FileChunk {
                ...
                let data_len = data.len().min(MAX_CONTROL_PAYLOAD - 8);
                let payload_len = 8 + data_len;
                let mut buf = vec![0u8; CONTROL_HEADER_SIZE + payload_len];
                buf[0] = Self::TYPE_FILE_CHUNK;
                buf[1..3].copy_from_slice(&(payload_len as u16).to_be_bytes());
                let p = CONTROL_HEADER_SIZE;
                buf[p..p + 4].copy_from_slice(&transfer_id.to_be_bytes());
                buf[p + 4..p + 8].copy_from_slice(&chunk_index.to_be_bytes());
                buf[p + 8..p + 8 + data_len].copy_from_slice(&data[..data_len]);
                buf
            }
            ControlMessage::FileComplete {
                ...
                let mut buf = vec![0u8; CONTROL_HEADER_SIZE + 40];
                buf[0] = Self::TYPE_FILE_COMPLETE;
                buf[1..3].copy_from_slice(&40u16.to_be_bytes());
                let p = CONTROL_HEADER_SIZE;
                buf[p..p + 4].copy_from_slice(&transfer_id.to_be_bytes());
                buf[p + 4..p + 8].copy_from_slice(&total_chunks.to_be_bytes());
                buf[p + 8..p + 40].copy_from_slice(sha256);
                buf
            }
            ControlMessage::FileCancel { transfer_id } => {
                let mut buf = vec![0u8; CONTROL_HEADER_SIZE + 4];
                buf[0] = Self::TYPE_FILE_CANCEL;
                buf[1..3].copy_from_slice(&4u16.to_be_bytes());
                buf[CONTROL_HEADER_SIZE..CONTROL_HEADER_SIZE + 4]
                    .copy_from_slice(&transfer_id.to_be_bytes());
                buf
            }
            ControlMessage::FileProgress {
                ...
                let mut buf = vec![0u8; CONTROL_HEADER_SIZE + 8];
                buf[0] = Self::TYPE_FILE_PROGRESS;
            ...
            ControlMessage::ClientBitratePreference(max_kbps) => {
                let mut buf =
                    vec![0u8; CONTROL_HEADER_SIZE + CLIENT_BITRATE_PREFERENCE_PAYLOAD_SIZE];
                buf[0] = Self::TYPE_CLIENT_BITRATE_PREFERENCE;
                buf[1..3].copy_from_slice(
                    &(CLIENT_BITRATE_PREFERENCE_PAYLOAD_SIZE as u16).to_be_bytes(),
                );
                buf[CONTROL_HEADER_SIZE
                    ..CONTROL_HEADER_SIZE + CLIENT_BITRATE_PREFERENCE_PAYLOAD_SIZE]
    ...
    pub fn deserialize(buf: &[u8]) -> Option<(Self, usize)> {
        ...
        let msg = match msg_type {
            ...
            Self::TYPE_CLIENT_BITRATE_PREFERENCE => {
                if payload.len() < CLIENT_BITRATE_PREFERENCE_PAYLOAD_SIZE {
                    return None;
                }
...
mod tests {
    ...
    #[test]
    fn roundtrip_transport_feedback_cc_fields() {
        let fb = TransportFeedback {
            ...
            completed_frames: 60,
            dropped_frames: 1,
            lost_frame_first: 4242,
            lost_frame_last: 4244,
            rtt_ms: 27,
            owd_trend_us: -1500,
            recv_video_kbps: 18_500,
        };
        let msg = ControlMessage::TransportFeedback(fb);
        let buf = msg.serialize();
    ...
    #[test]
    fn roundtrip_client_bitrate_preference() {
        let msg = ControlMessage::ClientBitratePreference(35_000);
        let buf = msg.serialize();
        let (decoded, consumed) = ControlMessage::deserialize(&buf).unwrap();
        assert_eq!(decoded, msg);
        assert_eq!(consumed, buf.len());
    ...
    #[test]
    fn roundtrip_client_display_info_hdr() {
        let mut info = ClientDisplayInfo {
            max_refresh_millihz: 144_000,
            udp_port: 5000,
            hdr_display: true,
            ..Default::default()
        };
        info.supported_video_codecs.insert(VideoCodec::H264);
        let msg = ControlMessage::ClientDisplayInfo(info);
        let buf = msg.serialize();
        let (decoded, _) = ControlMessage::deserialize(&buf).unwrap();
        assert_eq!(decoded, msg);
        // A short payload is rejected outright (no length tolerance).
    ...
    #[test]
    fn roundtrip_file_cancel() {
        let msg = ControlMessage::FileCancel { transfer_id: 99 };
        let buf = msg.serialize();
        let (decoded, consumed) = ControlMessage::deserialize(&buf).unwrap();
        assert_eq!(decoded, msg);
        assert_eq!(consumed, buf.len());
